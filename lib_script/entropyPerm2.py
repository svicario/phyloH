...
from DiversityTreeKL import cleanTree, TreeKL
...
class DBdata:
    ...
    def fromSimulator(self, filename, N=10000, ambienti=2,replicates=2, bonus=0.1,Xs=[], prob=None):
        if not Xs:
            for a in range((ambienti-1)):
                X=SelectNode(nodes)
                Xs.append(X)
        names,DB,Ns, table, t=FakeCommunity(Tree(filename),10000, ambienti=ambienti,replicates=replicates, bonus=bonus, Xs=Xs, prob=prob)
        self.countTable=DB
        self.SeqName=names
    ...
    def compressTable(self):
        ...
        for i,name in zip(T,self.SeqName):
            table[name]=i
        return table
    def GetEntropies(self, record=True, Perm=False, q=1 ):
        T=self.compressTable()
        groups={1:range(self.countTable.shape[1])}
        if self.withGroups:
            groups=self.groups
        try:
            H=TreeKL(t=self.tree, table=T, groups=groups,q=q, record=record, Perm=Perm)
        except KeyError:
