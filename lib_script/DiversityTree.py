...
def getSignNodeMult(t, alpha=0.05, rewrite=True):
    ...
    res=sorted(Sign, key=lambda x: x.pvalue, reverse=True)
    m=float(len(res))
...
def TreeKL(t, table, groups, q=1,record=True, Perm=False):
    ...
    return {"Hgamma":Hgamma, "HalphaBySamples":HalphaC, "MI_treeAndEnvironment":betaA, "MI_treeAndSampleGivenEnvironment":betaCA, "HalphaByEnvironment":HalphaA, "HgammaEachEnvironment":HalphaAA}
