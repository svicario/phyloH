...
def getAbbundance(name,table=None):
    ...
    try:
        ...
        L=len(table[table.keys()[0]])
        return numpy.array([0]*L)
...
def get_ROC(t,simNodes, bins):
    res=[]
    for alpha in range(0,bins):
        S=getSignNodeMult(t, alpha/float(bins), rewrite=True)
        R=getFPFN(t,simNodes, S)
        res.append([alpha/float(bins), R["sensitivity"],1-R["specificity"]])
    return res
              
def TreeKLOverall(t, table, groups, q=1, record=True, Perm=False):
...
def TreeKL(t, table, groups, q=1,record=True, Perm=False, branchScore=True):
    """
    ...
    """
    if not branchScore:
        TreeKLOverall(t=t, table=table, groups=groups, q=q, record=record, Perm=Perm)
    #print "sonoFabris", __name__, table
    ...
    Hgamma=sum([-x.dist*x.atot*log(x.atot) for x in t.traverse() if x.atot!=0])/t.T["Ttot"]
    HalphaA=Hgamma-betaA
